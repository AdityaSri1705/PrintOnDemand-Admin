 ...
 /*------------------ Supportive functions ---------------------------*/
  ...
  exports.generatePDF = async (req, res) => {
    ...
    try {
      ...
      console.log("postData=>",postData,pdfOptions);
      ...
      let pdf_content = { content: htmlContent };

      html_to_pdf.generatePdf(pdf_content, pdfOptions).then(pdfBuffer => {
        // Save the PDF buffer to a file
        fs1.writeFile(pdfPath, pdfBuffer, (err) => {
          if (err) {
            console.error('Error writing PDF file:', err);
            res.status(500).json({ status: false, result: "", errors: 'Error writing PDF file' });
          } else {
  ...
  async function readHtmlTemplate(data) {
    ...
    //layout images
    var ImageArr = {};
    const block_sql = `SELECT id, print_left_image, print_right_image  FROM layout_options`;
    var optList = await db.query(block_sql);
    optList.forEach((opt) => {
      ImageArr[opt.id] = {'left_image': baseUrl+opt.print_left_image, 'right_image': baseUrl+opt.print_right_image}; // Assign image to the key based on id
    });

    const CalenderTemplates = {};
    const calender_sql = `SELECT id, print_image, print_image2  FROM calendar_templates`;
    var CalenderTemplateList = await db.query(calender_sql);
    CalenderTemplateList.forEach((template) => {
      CalenderTemplates[template.id] = {'left_image': baseUrl+template.print_image, 'right_image': baseUrl+template.print_image2}; // Assign image to the key based on id
    });

    const AddinsTemplates = {};
    const addins_sql = `SELECT id, print_image, print_image2  FROM addins_templates `;
    var AddinsTemplateList = await db.query(addins_sql);
    AddinsTemplateList.forEach((template) => {
      AddinsTemplates[template.id] = {'left_image': baseUrl+template.print_image, 'right_image': baseUrl+template.print_image2}; // Assign image to the key based on id
    });
...
console.log(startDate, endDate, FirstPageData, LayoutData, CalendarData, AddinsData, DatesData)
    ...
    while (currentDate <= endDate) {
      ...
      {
        if(pageCount%2==0){
          console.log("Renewal HTML")
          var RenewalHTML = getRenwalTemplateHTML(ImageArr);
  ...
  function getRenwalTemplateHTML(ImageArr)
  {
    const rewaltemplateId = 320;
    const RenewalPageTemplate = path.join(__dirname, '../../views/DiaryPages', `RenewalPage.ejs`);
    const RenewalPageHTML = getSimplePageHTML(RenewalPageTemplate, ImageArr[rewaltemplateId]);
