...
const updateAddinsPath = async(item)=>{
 ...
 /*------------------ Supportive functions ---------------------------*/
  ...
  exports.getAddins = async (req, res) => {
    ...
    try {
        if(type==="Reflection"){
          const weekly_sql = `SELECT * FROM addins_templates WHERE type=? AND status='1'  ORDER BY id ASC`;
          var WeeklyList = await db.query(weekly_sql, ['Weekly-Reflection']);
          WeeklyList = await updateAddinsPath(WeeklyList);

          const monthly_sql = `SELECT * FROM addins_templates WHERE type=? AND status='1'  ORDER BY id ASC`;
          var MonthlyList = await db.query(monthly_sql, ['Monthly-Reflection']);
          MonthlyList = await updateAddinsPath(MonthlyList);

          templateList = { WeeklyReflection: WeeklyList, MonthlyReflection:MonthlyList};
  ...
  exports.getHolidayDates = async (req, res) => {
    ...
    try {
      ...
      const date_sql = `SELECT MIN(id) as id, title, holiday_id FROM holiday_dates GROUP BY holiday_id, title  ORDER BY holiday_id, event_date ASC`
